...
if ($_SERVER["REQUEST_METHOD"] == "POST") 
{
    ...
    $sqltugasanpelajar = "SELECT * FROM tugasan_pelajar WHERE ID_PELAJAR = ".$idpelajar." AND ID_TUGASAN_GURU = ".$_SESSION["ID_TUGASAN_GURU"]."";
        
    $subjek = (int)$_SESSION["idsubjeksemasa"];
    $stmt_tugasan = $conn->prepare($sqltugasanpelajar);
    $stmt_tugasan -> execute();
    $i = 0;
    ...
    <div id="wrapper">
        ...
        <div class="d-flex flex-column" id="content-wrapper">
            <div id="content">
                ...
                <div class="container">
                <form class="border rounded shadow" method = "POST" action = "<?php echo $_SERVER['PHP_SELF'];?>" enctype="" style="padding: 10%; ">
                    <div class="table-responsive table mt-2" id="dataTable-3" style = "overflow-x:auto;" role="grid" aria-describedby="dataTable_info">
                        <table class="w3-table-all" id="dataTable">
                            <thead>
                                <tr>
                                    <th>Bil.</th>
                                    <th>Markah</th>
                                    <th class="w3-center">Papar Tugasan</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php 
                                    ...
                                    $stmt_tugasan -> execute();
                                    $i = 0;
                                while($row = $stmt_tugasan->fetch()){ ?>
                                <tr>
                                    <td><?php echo ++$i; ?></td>
                                    <td><?php if(empty($row['MARKAH'])){
                                        echo 'Belum Dinilai';
                                    }else{
                                        echo $row['MARKAH'];
                                    } ?></td>
                                    <td class="w3-center"><a type="button" class="btn" href = "./<?php echo $row['TUGASAN'];?> " target="_blank" style="margin-right: 3px;color: var(--bs-white);background: rgb(78,115,223);border-color: var(--bs-blue);">Papar Tugasan</a></td>
                                </tr>
                                <?php } ?>
                            </tbody>
                        </table>
