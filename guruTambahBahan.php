...
{
    $tajuk = $_POST['Tajuk'];
    $dokumen = "simpanBahanPembelajaran/"; 
    $dokumen_file = $dokumen . basename($_FILES["Nota"]["name"]);
    $tarikh = $_POST['Tarikh'];
    $id = $_POST['id'];


    if(isset($tajuk) && !empty($dokumen) && !empty($tarikh) ){
        
    $sql2 = "INSERT INTO bahan_pembelajaran(ID_SUBJEK, ID_GURU, TAJUK_BAHAN, NOTA, TARIKH)
            VALUES (:ID_SUBJEK, :ID_GURU, :TAJUK_BAHAN, :NOTA, :TARIKH)";
    $stmt = $conn->prepare($sql2);
    $stmt->bindParam(":ID_SUBJEK", $id);
    $stmt->bindParam(":ID_GURU", $idguru);
    $stmt->bindParam(":TAJUK_BAHAN", $tajuk);
    $stmt->bindParam(":NOTA", $dokumen_file);
        ...
        <div class="d-flex flex-column" id="content-wrapper">
            <div id="content">
                ...
                <div style="background: #1a3c63;padding: 2%;">
                    <h1 class="text-center" style="font-weight: bold;font-size: 50px;color: rgb(255,255,255);">Bahan Pembelajaran</h1>
                </div>
                <div class="container">
                <?php
                            /*include ("./includes/connect.php");

                            $sql = "SELECT * FROM bahan_pembelajaran WHERE ID_SUBJEK = :idsubjek";
                            $stmt = $conn->prepare($sql);
                ...
                <form class="border rounded shadow" method = "POST" action = "<?php echo $_SERVER['PHP_SELF'];?>" enctype="multipart/form-data" style="padding: 10%; ">
                        <div class="card shadow">
                            <div class="card-header py-3">
                                <p class="text-primary m-0 fw-bold">Tambah Bahan Pembelajaran</p>
                            </div>
                            <div class="card-body">
                            <!--<div><input class="d-none" name="id" value="<?php //if($row = $stmt->fetch()){ echo $row['ID_SUBJEK']; ?>"><label class="form-label">Tajuk Bahan:</label><input class="form-control" name = "Tajuk" type="text" value="<?php //echo $row['TAJUK_BAHAN'];} ?>"></div><br>-->
                            <div><input class="d-none" name="id" value="<?php { echo $_GET['subId'];} ?>"><label class="form-label">Tajuk Bahan:</label><input class="form-control" name = "Tajuk" type="text" value="<?php //echo $row['TAJUK_BAHAN'];} ?>"></div><br>
   
                                <div><label class="form-label">Nota:</label><input class="form-control" name = "Nota" type="file"></div><br>
                                <div><label class="form-label">Tarikh:</label><input class="form-control" name = "Tarikh" type="date"></div><br>
                                <div></div><br><br>
                                <div></div><br>
                                <div><button class="btn btn-secondary float-start" href="guruMatapelajaran.php" type="reset">Set Semula</button><button class="btn btn-primary float-end" type="submit">Simpan</button>
