...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(isset($tajuk) && isset($penerangan) && !empty($dokumen) && !empty($tarikh)){
        ...
        $stmt->bindParam(":ID_PENGUMUMAN", $id);
        $stmt->execute();
            ...
            <div id="content">
                ...
                <div class="container">
                        ...
                        <?php
                            ...
                            $sql = "SELECT * FROM pengumuman WHERE ID_PENGUMUMAN = :idpengumuman";
                            $stmt = $conn->prepare($sql);
                ...
                <form class="border rounded shadow" method = "POST" action = "<?php echo $_SERVER['PHP_SELF'];?>" enctype="multipart/form-data" style="padding: 10%; ">
                        <div class="card shadow">
                            <div class="card-header py-3">
                                <p class="text-primary m-0 fw-bold">Kemaskini Pengumuman</p>
                            </div>
                            ...
                            <div><input class="d-none" name="id" value="<?php /*while($row = $stmt->fetch()){*/ echo $row['ID_PENGUMUMAN']; ?>"><label class="form-label">Tugasan (Sila Masukkan Semula Fail Tugasan):</label><input class="form-control" name = "dokumen" type="file" value="<?php echo $row['DOKUMEN']; ?>" placeholder="Isi Fail"></div><br>
