<?php
   ...
   if(isset($_GET["idpelajar"])){
    $_SESSION["idpelajar"]= $_GET["idpelajar"];
   }

  if (isset($_POST['hantar'])){
    if(!empty($_POST['NamaPelajar']) && !empty($_POST['UmurPelajar']) && !empty($_POST['TarikhLahirPelajar']) && !empty($_POST['MyKid']) 
    && !empty($_POST['Jantina'] ) && isset($_POST['Agama']) && isset($_POST['Bangsa'])){
        
    $namapelajar = $_POST['NamaPelajar'];
    $umurpelajar = $_POST['UmurPelajar'];
    $tarikhlahirpelajar = $_POST['TarikhLahirPelajar']; 
    $mykid = $_POST['MyKid'];
    $jantina = $_POST['Jantina'];
    $agama = $_POST['Agama'];
    $bangsa = $_POST['Bangsa'];
    //header("Location: ./adminKemaskiniPelajarPenjaga.php");
    //$idpelajar = $_POST['idpelajar'];

    // $_SESSION["namapelajar"] = $namapelajar;
    // $_SESSION["umurpelajar"] = $umurpelajar;
    // $_SESSION["tarikhlahirpelajar"] = $tarikhlahirpelajar;
    // $_SESSION["mykid"] = $mykid;
    // $_SESSION["jantina"] = $jantina;
    // $_SESSION["agama"] = $agama;
    // $_SESSION["bangsa"] = $bangsa;
    //     echo $_SESSION["namapelajar"];
    $sql1 = "UPDATE pelajar SET NAMA_PELAJAR = '".$namapelajar."', UMUR='".$umurpelajar."', TARIKH_LAHIR='".$tarikhlahirpelajar."', NO_MYKID='".$mykid."', JANTINA='".$jantina."', AGAMA='".$agama."', BANGSA='". $bangsa."'
    WHERE ID_PELAJAR = ".$_SESSION["idpelajar"]."";
  
  //echo $_SESSION["namapelajar"];
  $stmt = $conn->prepare($sql1);
//   $stmt->bindParam(":NAMA_PELAJAR", $namapelajar);
//   $stmt->bindParam(":UMUR", $umurpelajar);
//   $stmt->bindParam(":TARIKH_LAHIR", $tarikhlahirpelajar);
//   $stmt->bindParam(":NO_MYKID", $mykid);
        ...
        <div id="layoutSidenav">
            ...
            <div id="layoutSidenav_content">
                <div style="background: #1a3c63;padding: 2%;">
                    <h1 class="text-center" style="font-weight: bold;font-size: 50px;color: rgb(255,255,255);">Kemaskini Pelajar</h1>
                </div>
                <div class="container">
                    <div class="row">
                        <div class="col-xxl-8 offset-xxl-2">
                        <?php
                            $sql = "SELECT * FROM pelajar WHERE ID_PELAJAR = :idpelajar";
                            $stmt = $conn->prepare($sql);
                            $stmt->bindParam(":idpelajar", $_SESSION["idpelajar"]);
                            $stmt->execute();
                        ?>
                            <form class="border rounded shadow" method = "post" action = "./adminKemaskiniPelajar.php" style="padding: 10%;">
                            <?php while ($row = $stmt->fetch()){?> 
                                <div><input type = "hidden" name = "idpelajar" value = <?php echo $_SESSION["idpelajar"];?>> 
                                <input class="d-none" name="id" value="<?php  echo $row['ID_PELAJAR']; ?>"><label class="form-label">Nama Penuh:</label><input value = "<?php  echo $row['NAMA_PELAJAR']; ?>" class="form-control" name = "NamaPelajar" type="text"></div><br>
                                <div><label class="form-label">Umur:</label><input value = "<?php  echo $row['UMUR']; ?>" class="form-control" name = "UmurPelajar" type="text"></div><br>
                                <div><label class="form-label">Tarikh Lahir:</label><input value = "<?php  echo $row['TARIKH_LAHIR']; ?>" class="form-control" name = "TarikhLahirPelajar" type="date"></div><br>
                                <div><label class="form-label">No. MyKid:</label><input value = "<?php  echo $row['NO_MYKID']; ?>" class="form-control" name = "MyKid" type="text"></div><br>
                                <div><label class="form-label">Jantina:</label>
                                    <div> <input class="d-none" name="id" value="<?php  echo $row['ID_PELAJAR']; ?>" class="form-check"><input value = "Lelaki" value = "<?php  echo $row['JANTINA']; ?>" class="form-check-input" name = "Jantina" type="radio" id="formCheck-1" <?php if(strcmp($row['JANTINA'],"Lelaki")==0){echo 'checked = "checked"';}?> ><label class="form-check-label" for="formCheck-1">Lelaki</label></div>
                                    <div> <input class="d-none" name="id" value="<?php  echo $row['ID_PELAJAR']; ?>" class="form-check"><input value = "Perempuan" value = "<?php  echo $row['JANTINA']; ?>"class="form-check-input" name = "Jantina" type="radio" id="formCheck-2" <?php if(strcmp($row['JANTINA'],"Perempuan")==0){echo 'checked = "checked"';}?> ><label class="form-check-label" for="formCheck-2">Perempuan</label></div>
                                </div><br>
                                <div><label class="form-label">Agama:</label><select name = "Agama" class="form-select" value="<?php  echo $row['AGAMA']; ?>">
                                        
                                        <option value="Islam">Islam</option>
                                        <option value="Buddha">Buddha</option>
                                        <option value="Kristian">Kristian</option>
                                    </select></div><br>
                                <div><label class="form-label">Bangsa:</label><input value = "<?php  echo $row['BANGSA']; ?>" class="form-control" name = "Bangsa" type="text"></div><br>
                                <?php }?>

                                <div><button class="btn btn-secondary float-start" type="reset" >Set Semula</button><button class="btn btn-primary float-end" type="submit" name="hantar">Seterusnya</button></div>
                            </form>
                        </div>
                    </div>
                </div>
