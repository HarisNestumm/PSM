...
if ($_SERVER["REQUEST_METHOD"] == "POST") 
{
    ...
    echo "<script>
            alert('Bahan Pembelajaran Berjaya Dikemaskini.');
        ...
        <div class="d-flex flex-column" id="content-wrapper">
            <div id="content">
                ...
                <?php
                            ...
                            $sql = "SELECT * FROM bahan_pembelajaran WHERE ID_BAHAN = :idbahan";
                            $stmt = $conn->prepare($sql);
                ...
                <form class="border rounded shadow" method = "POST" action = "<?php echo $_SERVER['PHP_SELF'];?>" enctype="multipart/form-data" style="padding: 10%; ">
                        <div class="card shadow">
                            <div class="card-header py-3">
                                <p class="text-primary m-0 fw-bold">Kemaskini Bahan Pembelajaran</p>
                            </div>
                            <div class="card-body">
                            <?php while ($row = $stmt->fetch()){?> 
                                <div><input class="d-none" name="id" value="<?php  echo $row['ID_BAHAN']; ?>"><label class="form-label">Tajuk Bahan:</label><input class="form-control" name = "Tajuk" type="text" value="<?php echo $row['TAJUK_BAHAN']; ?>"></div><br>
                                <div><input class="d-none" name="id" value="<?php  echo $row['ID_BAHAN']; ?>"><label class="form-label">Nota (Sila muat naik yang baharu):</label><input class="form-control" name = "Nota" type="file" value="<?php echo $row['NOTA']; ?>" placeholder="Isi Fail"></div><br>
                                <div><input class="d-none" name="id" value="<?php echo $row['ID_BAHAN']; ?>"><label class="form-label">Tarikh:</label><input class="form-control" name = "Tarikh" type="date" value="<?php echo $row['TARIKH'];?>"></div><br>
                                <?php }?>
                                <div></div><br><br>
                                <div></div><br>
                                <div><button class="btn btn-primary float-end" type="submit">Simpan</button>
